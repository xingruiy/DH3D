...
def desc_local_loss(outs_dict, pos_r=0.5, search_r=20, margin=0.8, extra=False, neg_weight=5, **kwargs):
    ...
    is_neg = tf.cast(tf.logical_and(is_out_of_safe_radius,
                                    is_within_search_radius), dtype=tf.float32)
...
def local_detection_loss_nn(outs_dict, ar_th=0.3, det_k=16, ar_nn_k=5, pos_r=0.3, use_hardest_neg=True, **unused):
    ...
    sample_ind0, sample_ind1 = tf.split(
        outs_dict['sample_nodes_concat'], 2, axis=0)
    ...
    if use_hardest_neg:
        ...
        hardest_neg_indices = tf.concat(
            [batch_indices, hardest_neg_ind1], axis=-1)  # Batch, M, 2
        # batch, numpts, k ===> batch, M, k
        knn_sampled_neg1 = tf.gather_nd(knn1, hardest_neg_indices)
        knn_sampled1 = tf.concat([knn_sampled1, knn_sampled_neg1], -1)
        det_k = det_k * 2

    # gather feat
    batch_indices = tf.tile(tf.reshape(tf.range(
        batchsize), (-1, 1, 1, 1)), (1, samplenum, det_k, 1))  # N samplenum k,1
    feat1_indices = tf.concat([batch_indices, tf.expand_dims(
        knn_sampled1, axis=3)], axis=-1)  # N m k 2
    ...
    samplenum_indices = tf.tile(tf.reshape(
        tf.range(samplenum), (1, -1, 1, 1)), (batchsize, 1, ar_nn_k, 1))
    indices_k_feat_select = tf.concat([batch_indices, samplenum_indices, tf.expand_dims(indices_k_feat, axis=3)],
                                      axis=-1)
    # compute ar
    sampled_xyzdist_selected = tf.gather_nd(
        matching_xyz_dist, indices_k_feat_select)

    is_good = tf.cast(tf.less_equal(
        sampled_xyzdist_selected, pos_r), tf.float32)
    padones = tf.ones(
...
def lazy_quadruplet_loss(global_descs, batch_size, num_pos, num_neg, global_triplet_margin=0.5, global_quadruplet_margin=0.2, **kwargs):
    ...
    pos_vecs = tf.reshape(posvec, [batch_size, num_pos, outdim], name='posvec')
    neg_vecs = tf.reshape(negvec, [batch_size, num_neg, outdim], name='negvec')
    other_neg_vec = tf.reshape(
        othernegvec, [batch_size, 1, outdim], name='othernegvec')

    trip_loss = lazy_triplet_loss_impl(
        q_vec, pos_vecs, neg_vecs, global_triplet_margin)
