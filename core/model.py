...
class DH3D(ModelDesc):
    ...
    def inputs(self):
        ...
        if self.config.input_R:
            ret.append(tf.TensorSpec(
                (self.config.batch_size, 3, 3), tf.float32, 'R'))
        ...
        if self.config.sampled_kpnum > 0:
            ...
            ret.append(tf.TensorSpec((self.config.batch_size,
                                      self.config.sampled_kpnum), tf.int32, 'sample_ind_pos'))
        ...
        if self.config.num_points > 8192:
            ...
            if self.config.num_pos > 0:
                ret.append(tf.TensorSpec(
                    (self.config.batch_size, self.config.num_points *
                     self.config.num_pos, self.config.knn_num),
                    tf.int32,
                    'knn_ind_pos'))
            if self.config.num_neg > 0:
                ret.append(tf.TensorSpec(
                    (self.config.batch_size, self.config.num_points *
                     self.config.num_neg, self.config.knn_num),
                    tf.int32,
                    'knn_ind_neg'))
        return ret
    ...
    def compute_global(self, outs, freeze_global=False):
        with backbone_scope(freeze=freeze_global):
            points = outs['xyz']
            localdesc = outs['feat']
            newpoints, forglobal = getattr(backbones, self.global_backbone)(
                points, localdesc, **self.config)
            ...
            globaldesc = getattr(
                backbones, self.global_assemble)(**inputs_dict)
        return globaldesc
    ...
    def build_graph(self, *inputs_dict):
        ...
        if self.config.sampled_kpnum > 0:
            sample_nodes_concat = tf.concat(
                [inputs_dict['sample_ind_anchor'], inputs_dict['sample_ind_pos']], 0)
        ...
        saved_tensor_xyz_feat = tf.concat(
            [newpoints, localdesc_l2normed], -1, name='xyz_feat')

        # get local attentions
        if self.config.detection:
            detect_att = getattr(backbones, self.detection_block)(
                localdesc, freeze_det=freeze_det)
            outs['attention'] = detect_att
            saved_tensor_xyz_feat_att = tf.concat(
                [newpoints, localdesc_l2normed, detect_att], -1, name='xyz_feat_att')
        ...
        if self.config.sampled_kpnum > 0:
            ...
            xyz_feat = tf.concat(
                [localxyzsample, localfeatsample], -1, name='xyz_feat_sampled')
            if self.config.get('detection'):
                att_sampled = tf.squeeze(group_point(
                    detect_att, kp_indices), axis=-1)
    ...
    def compute_loss(self, outs):
        ...
        # global loss
        if self.config.extract_global:
            global_loss = getattr(losses, self.global_loss_func)(
                global_descs=outs['global_desc'], **self.config)
            global_loss = tf.multiply(
                global_loss, self.config.global_loss_weight, name='globaldesc_loss')
            add_moving_summary(global_loss)
            loss += global_loss

        # local loss
        if self.config.add_local_loss:
            local_loss = getattr(losses, self.local_loss_func)(
                outs, **self.config)
            local_loss = tf.multiply(
                local_loss, self.config.local_loss_weight, name='localdesc_loss')
            add_moving_summary(local_loss)
            loss += local_loss

        # detection loss
        if self.config.detection and self.config.add_det_loss:
            det_loss = getattr(losses, self.detection_loss_func)(
                outs, **self.config)
            det_loss = tf.multiply(
                det_loss, self.config.det_loss_weight, name='det_loss')
